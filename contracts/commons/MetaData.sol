...
contract MetaData is AccessControl, IMetaData {
  ...
  // Props
  function addPower(uint256 prop, uint256[] calldata _power) override external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function addPowerBatch(uint256[] calldata props, uint256[][] calldata _powers) override external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function getPower(uint256 prop) view override external returns(NFTPartProp memory) {
    return powers[prop];
  }

  function getPowerTokenId(uint256 tokenId) view external override returns(NFTPartProp memory attack, NFTPartProp memory defend, uint256 total) {
    (attack, defend) = _powerTokenId(tokenId, attack, defend);
    total = _totalProps(attack);
    total += _totalProps(defend);
  }

  function _powerTokenId(uint256 tokenId, NFTPartProp memory attack, NFTPartProp memory defend) view internal returns(NFTPartProp memory, NFTPartProp memory) {
  ...
  function getPowerTeam(uint256[] calldata tokenIds) view override external returns(NFTPartProp memory attack, NFTPartProp memory defend, uint256 total) {
  ...
  function getPowerTeamNotTotal(uint256[] memory tokenIds) view external override returns(NFTPartProp memory attack, NFTPartProp memory defend) {
   for (uint256 inx = 0; inx < tokenIds.length; inx++) {
      (attack, defend) = _powerTokenId(tokenIds[inx], attack, defend);
    }
  }
  
  function addEfficiency(uint256 prop, uint256[] calldata eff) override external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function addEfficiencyBatch(uint256[] calldata effPro, uint256[][] calldata effVals) override external onlyRole(DEFAULT_ADMIN_ROLE) {
    for (uint256 index = 0; index < effPro.length; index++) {
      _addEfficiency(effPro[index], effVals[index]);
    }
  }

  function getEfficiency(uint256 prop) view override external returns(KuniEfficiency memory) {
    return efficiencies[prop];
  }

  function getEfficiencyTokenId(uint256 tokenId) view override external returns(KuniEfficiency memory efficiency) {
    efficiency = _efficiencyTokenId(tokenId, efficiency);
  }

  function getEfficiencyTokenIds(uint256[] memory tokenIds) view override external returns(KuniEfficiency memory efficiency) {
    for (uint256 inx = 0; inx < tokenIds.length; inx++) {
  ...
  function _efficiencyTokenId(uint256 tokenId, KuniEfficiency memory efficiency) view internal returns(KuniEfficiency memory) {
    uint256[] memory props = sarus[tokenId];
  ...
  function getPowerAllTokenId(uint256 tokenId) view override external returns(NFTPartProp memory power, uint256 total) {
    uint256[] memory props = sarus[tokenId];
    for (uint256 j = 0; j < props.length; j++) {
      NFTPartProp memory tmp = powers[props[j]];
  ...
  function getPowerAllTeam(uint256[] calldata tokenIds) view override external returns(NFTPartProp memory power, uint256 total) {
    for (uint256 inx = 0; inx < tokenIds.length; inx++) {
   ...
   */
  function addNft(uint256 _tokenId, uint256[] memory props) override external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  /*
    ...
    - props: hand, weapon, head, eyes, body, hair, cat
  */ 
  function addNftBatch(uint256[] memory tokenIds, uint256[][] memory props) external override onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  // SLASH; HEAVY; STRIKE; TECH; MAGIC;
  function addMaterials(uint256[] memory pic, uint256[][] memory data) external override onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function addItemCraft(string[] memory names, uint256[][] memory data) external override onlyRole(DEFAULT_ADMIN_ROLE) {
    for (uint256 index = 0; index < data.length; index++) {
      // name, cat;
