...
contract Scholarship is IERC721Receiver, IScholarship, ReentrancyGuard, Ownable {
    ...
    function cancel(address nftAddr, uint256 tokenId) external override nonReentrant {
    ...
    function ownerInfo(address nftAddr, uint256 tokenId) external view override returns (address, uint256) {
        ScholarInfo memory info = rateOf[nftAddr][tokenId];
    ...
    function getScholars(
        ...
        uint256 start,
        uint256 limit
    ) external view virtual returns (uint256[] memory, address[] memory, uint256[] memory) {
