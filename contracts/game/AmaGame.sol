    ...
    ReentrancyGuard
{
    ...
    function _invalidSaru(uint256[] calldata tokenIds, address sender) internal view returns (bool) {
      for (uint256 index = 0; index < tokenIds.length; index++) {
        ...
        require(IERC721(kuniSaru).ownerOf(tokenId) == sender || _nftOwner[tokenId] == sender || sOwner != address(0x0), "KUNI: Your not is owner");
      }
      return true;
    }
    ...
    function _mUpdatePool(address mToken) internal {
      ...
      pool.rewardPerShare = pool.rewardPerShare.add(
        rewardForMiner.mul(MAGIC_NUM).div(pool.supply)
      );
      pool.lastRewardBlock = block.number;
    }
    ...
    function pendingReward(address mToken, address sender) external view returns (uint256) {
      ...
      if (user.amount > 0) {
        ...
        uint256 share = pool.rewardPerShare.add(
          rewardForMiner.mul(MAGIC_NUM).div(pool.supply)
        );
