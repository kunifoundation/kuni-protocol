...
describe('------------- Staking token ------------------', () => {
  ...
  it("01. Scholarship ASK", async function () {
    ...
    let tokenIds = (await Promise.all(_.range(0, Number(total)).map(index => this.kuniSaru.tokenOfOwnerByIndex(alex.address, index)))).map(t => Number(t))
    console.log(tokenIds.map(t => toNumber(t)))

    total = await this.kuniSaru.balanceOf(bob.address)
     tokenIds = (await Promise.all(_.range(0, Number(total)).map(index => this.kuniSaru.tokenOfOwnerByIndex(bob.address, index)))).map(t => Number(t))
